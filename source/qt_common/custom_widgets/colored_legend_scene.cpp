    ...
    , legend_mode_(LegendMode::kColor)
{
    ...
    connect(&QtCommon::QtUtils::ColorTheme::Get(), &QtCommon::QtUtils::ColorTheme::ColorThemeUpdated, this, &ColoredLegendScene::Update);
}
...
ColoredLegendScene::~ColoredLegendScene()
{
    ...
    disconnect(&QtCommon::QtUtils::ColorTheme::Get(), &QtCommon::QtUtils::ColorTheme::ColorThemeUpdated, this, &ColoredLegendScene::Update);
}
...
void ColoredLegendScene::Update()
{
    ...
    if (legend_mode_ == LegendMode::kColor)
    {
        for (int i = 0; i < color_legends_.size(); i++)
        {
            ...
            item.text_item_->setDefaultTextColor(QtCommon::QtUtils::ColorTheme::Get().GetCurrentThemeColors().graphics_scene_text_color);
    ...
    else if (legend_mode_ == LegendMode::kText)
    {
        for (int i = 0; i < text_legends_.size(); i++)
        {
            ...
            text_legends_[i]->setDefaultTextColor(QtCommon::QtUtils::ColorTheme::Get().GetCurrentThemeColors().graphics_scene_text_color);
